...
/*Konstanten für das Abbilden der 3D Koordinaten auf Texturkoordinaten*/
...
void
perspective(float fov, float aspect, float znear, float zfar, float* m)
{
...
void
checkShaderLog(const char* filename, GLuint shader)
{
  GLint length;
...
GLuint
createShader(GLenum shaderType, const char* filename)
{
...
void
checkProgramLog(GLuint program)
{
  GLint length;
...
static float
mapRange(float a1, float a2, float b1, float b2, float s)
{
  return b1 + (s - a1) * (b2 - b1) / (a2 - a1);
}

float
coordToTex(float coord)
{
  return mapRange(
    COORD_RANGE_A, COORD_RANGE_B, TEX_RANGE_A, TEX_RANGE_B, coord);
}

void
loadTexture(char* path, GLuint* bindLoc)
{
  int width, height, comp;
  GLubyte* data = stbi_load(path, &width, &height, &comp, 4);

  /* Erstellen des Textur-Objekts. */
  glGenTextures(1, bindLoc);
  glBindTexture(GL_TEXTURE_2D, *bindLoc);
