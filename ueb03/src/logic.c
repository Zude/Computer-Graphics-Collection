 ...
 */
void
setFieldAt(int x, int y, lvFieldType type)
{
  currentLevel[y][x] = type;
}
 ...
 */
void
advanceSugarTimer(double interval)
{
 ...
 */
fieldInformation
getNextField(int x, int y, CGDirection direction)
{
 ...
 */
void
performPlayerMovement(CGDirection direction,
                      fieldInformation nextField,
                      Avatar* player)
{
...
void
movePlayer(CGDirection inputDir)
{
  // Wenn zwischen der Letzen Bewegung mindestens PLAYER_STEP_TIME vergangen ist
  if (!isPlayerDead && isStepAllowed && !win && !Pause && !ShowInstructions) {
...
/**
 * Berechnet die XZ Koordinaten für die gegebene Richtung
 * @param start XYZ Koordinaten
 * @param distance Entfernung der neuen Koordinaten
 * @param dir Richtung in der neuen Koordinaten, ausgehend vom Spielfeld
 */
float*
getDirCoords(float* start, float distance, CGDirection dir)
{
  static float goal[] = { 0.0f, 0.0f };

  switch (dir) {
    case dirLeft:
      goal[0] = start[0];
      goal[1] = start[2] + distance;
      break;
    case dirRight:
      goal[0] = start[0];
      goal[1] = start[2] - distance;
...
/**
 * Zeit abhängiger Wert für Animation
 * @return PLAYER_STEP_TIME * currentStepTime
 */
float
getAnimationDist(void)
{
  return PLAYER_STEP_TIME * currentStepTime;
}
