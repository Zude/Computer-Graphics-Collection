...
int getTexturingStatus(void) { return g_texturingStatus; }

void setTexturingStatus(int status) { g_texturingStatus = status; }
...
static int initTextureArray(void) {
  ...
  g_textures[texLand].filename = "../textures/land.jpg";
  g_textures[texWater].filename = "../textures/water.jpg";
  g_textures[texFace].filename = "../textures/face.jpg";
...
static int loadTextures(void) {
  ...
  if (initTextureArray()) {
    ...
    for (i = 0; i < TEX_COUNT; i++) {
      /* Die 4 erzwingt, dass das Bild als RGBA weitergereicht wird,
       * unabhängig von der Anzahl tatsächlich vorhandener Kanäle */
      data = stbi_load(g_textures[i].filename, &width, &height, &channels, 0);

      if (data != NULL) {
        glBindTexture(GL_TEXTURE_2D, g_textures[i].id);

        gluBuild2DMipmaps(GL_TEXTURE_2D, channels, width, height,
                          calculateGLBitmapMode(channels), GL_UNSIGNED_BYTE,
                          data);
